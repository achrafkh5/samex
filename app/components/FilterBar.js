...
const fuelTypes = [...new Set(carsData.map(car => car.fuelType).filter(Boolean))];
const transmissions = [...new Set(carsData.map(car => car.transmission).filter(Boolean))];
const conditions = [...new Set(carsData.map(car => car.condition).filter(Boolean))];
...
<button onClick={onReset} className="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium transition-colors" >
{t('resetFilters')}
</button>
</div>
  ...
  <div className="space-y-6">
    ...
    <div>
      ...
      >
        ...
        {conditions.map((condition, i) => (
          <option key={`${condition}-${i}`} value={condition}>
            {t(condition)}
          </option>
        ))}
      </select>
    ...
    <div>
      ...
      >
        ...
        {brands.map((brand, i) => (
          <option key={`${brand}-${i}`} value={brand}>{brand}</option>
        ))}
      </select>
    </div>
    ...
    <div>
      ...
      >
        ...
        {years.map((year, i) => (
          <option key={`year-${year}-${i}`} value={year}>{year}</option>
        ))}
      </select>
    </div>
    ...
    <div>
      ...
      >
        ...
        {transmissions.map((trans, i) => (
          <option key={`${trans}-${i}`} value={trans}>
            {t(trans)}
          </option>
        ))}
      </select>
